...
def main():
    ...
    train_options, hidden_config, noise_config = util.load_options(args.options_file)
    noiser = Noiser(noise_config)

    checkpoint = torch.load(args.checkpoint_file)
    hidden_net = Hidden(hidden_config, device, noiser, None)
    util.model_from_checkpoint(hidden_net, checkpoint)
    ...
    util.save_images(image_tensor.cpu(), encoded_images.cpu(), os.path.join('test-output', 'test-model.png'), resize_to=(256, 256))
