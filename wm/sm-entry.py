...
def create_sagemaker_run_parser():
    ...
    parser.add_argument('--enable-fp16', type=int, default=0,  help='Enable mixed-precision training.')
    ...
    parser.add_argument('--use-dropout', type=int, default=0, help='Whether to use dropout in Unet architectures')
...
def main():
    ...
    print(f'Parsed command line args:\n{pformat(args.__dict__)}')
    job = sm_jobman.SagemakerJobManager(args, command_line_args=' '.join(sys.argv[1:]))
    print('-'*80)
    pprint(job.config)
    job.start_or_resume()
