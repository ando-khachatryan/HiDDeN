...
class JpegCompression(nn.Module):
    def __init__(self, yuv_keep_weights = (25, 9, 9)):
        ...
        dct_conv_weights = torch.tensor(gen_filters(8, 8, dct_coeff), dtype=torch.float32)
        dct_conv_weights.unsqueeze_(1)
        self.register_buffer('dct_conv_weights', dct_conv_weights)
        idct_conv_weights = torch.tensor(gen_filters(8, 8, idct_coeff), dtype=torch.float32)
        idct_conv_weights.unsqueeze_(1)
        self.register_buffer('idct_conv_weights', idct_conv_weights)

        self.yuv_keep_weighs = yuv_keep_weights
        # self.keep_coeff_masks = []
        self.register_buffer('jpeg_mask', torch.empty((0, 0)))
        # self.jpeg_mask = None
    ...
    def create_mask(self, requested_shape):
        if self.jpeg_mask is None or requested_shape > self.jpeg_mask.shape[1:]:
            self.jpeg_mask = torch.empty((3,) + requested_shape, device=self.dct_conv_weights.device)
